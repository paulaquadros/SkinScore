...
function Produto ({produto}){
    ...
    useEffect(() => {
        const getImagem = async () => {
            try{
                await axios.get(`http://3.145.180.184:3001/produtos/${produto?.id}`, {headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` }}).then(
                    (response) => {
...
export default function BuscaResultados() {
    ...
    useEffect(() => {
        ...
        const getProdutos = async () => {
            try{
                await axios.get(`http://3.145.180.184:3001/produtos/search`, {params: {termo: busca}}, {headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` }})
        ...
        const getUsuario = async () => {
                try{
                    await axios.get(`http://3.145.180.184:3001/usuarios/search`, {params: {nome_usuario: busca}}, {headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` }})
